...
type
  ...
  public
    ...
    function CreateIPCServer(FileName: string; IPCServName : string; ShowCmd: DWORD): string;
...
function TIPCClient.CreateIPCServer(FileName: string; IPCServName : string; ShowCmd: DWORD): string;
var
...
begin
  ...
  begin
    // dima FClientThread := TClientThread.Create(Self);
...
var
  BufferSize, NumberOfBytesWritten, MessageCount: DWORD;
  MemoryStream: TMemoryStream;
  ClientNameData: TClientNameData;
...
begin
  ...
  //FCS.Enter;
  //try
    if GetMailslotInfo(ServerHandle, nil, BufferSize, @MessageCount, nil) then
    begin
...
function TIPCClient.DoReadData : boolean;
var
  Data: Pointer;
  ClientName: WideString;
begin
  Data := nil;
  Result := false;
  if ReadData(FResponseServerHandle, ClientName, Data) then
  begin
    if Data <> nil then
    begin
